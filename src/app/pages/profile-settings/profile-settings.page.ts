...
export class ProfileSettingsPage implements OnInit {
  ...
  constructor(
    ...
    this.form = new FormGroup({
      ...
      // Agrupamos las contraseñas para aplicar validadores personalizados
  ...
  submitForm() {
    ...
    this.apiService.updateCount(this.form.value).subscribe(async (result) => {
      this.isChargeLoading=false;
        // Emitir cambios y volver a la pantalla anterior
        this.apiService.userChanges2.next();
        this.navCtrl.pop();
       
        const toastMensaje = await this.translate.get("profile-settings.Cuenta de usuario actualizada").toPromise();
        this.utilitiesService.showToast(toastMensaje); 
