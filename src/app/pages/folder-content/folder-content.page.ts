...
export class FolderContentPage implements OnInit {
  ...
  ionViewWillEnter() {
    ...
    if(history.state.isVerifiedUser){
      this.isVerifiedUser=history.state.isVerifiedUser;
    }
    if(history.state.isAdmin){
      this.isAdmin=history.state.isAdmin;
    }
    ...
    if (!this.mediaActivatedSubscription) {
      this.mediaActivatedSubscription = this.apiService.mediaActivatedFolder.subscribe(funcion => {
        console.log("mediaActivatedSubscription");
        this.uploadMedia();
      });  
    }
  ...
  goBack(){
    console.log("(folder-content)Volviendo a la página anterior");
    //  M this.navController.pop();
...
//this.utilities.showLoading('');
  ...
  goNewFolderContent(){
    this.navController.navigateForward("/new-folder-content", {state: {folderId:this.folderId,projectId:this.projectId}});
  }
  ...
  public irAFolder(multimedia: any,$modal) {
   ...
   // console.log('funcion cerrar');
    this.isModalOpen2 = false;
      ...
      // Navega a la página "new-folder-content" con los mismos parámetros que usas en otros lugares
      this.navController.navigateForward("/new-folder-content", {
        state: {
          projectId: this.projectId,
