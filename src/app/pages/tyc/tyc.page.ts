...
export class TYCPage implements OnInit {
  ...
  ionViewDidEnter() {
    ...
    this.utilities.getLang().then((result) => {
      ...
      if (prefijo == null) {
       ...
       // this.pdfRoute = "assets/icons/CondicionesContratacion.pdf";  // Default PDF
      } else {
        if (prefijo == 'es') {
        //  this.pdfRoute = "assets/icons/CondicionesContratacion.pdf";
        } else {
           this.form.get('language_code').patchValue(prefijo);
         // this.pdfRoute = "assets/icons/ContractConditionsInsitu360_en.pdf";      
        }
      }
      ...
      this.apiService.getTerConTextValue(this.form.value).subscribe(async (result) => {
        console.log('terminos valor: ',result);
        if(result!=null){
  ...
  ionViewWillEnter() {
    ...
    this.routerSubscription = this.router.events.subscribe((event) => {
      if (event instanceof NavigationEnd) {
        ...
        if (!event.url.includes('/tyc')) {
         // console.log('Saliendo de tyc, limpiando intervalos');
          // this.clearInterval();
        }
        else{
          // this.startInterval();
  ...
  private clearInterval() {
    ...
    if (this.interval) {
      ...
      //ENVIO TIEMPO
      this.apiService.registerTimeScreen({screenId:29,screenTime:this.countSeg}).subscribe((result) => {
  ...
  private checkDateDifference() {
    ...
    if(!this.isActive2){
      ...
      console.log(`tyc: Diferencia en segundos: ${differenceInSeconds} , contador actual: `);
    ...
    if (this.isActive2) {
      ...
      console.log('tyc: No hay fecha anterior, usando la actual como inicial.');
    }
