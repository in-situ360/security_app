...
@Component({
  selector: 'app-modal-link-folder',
  templateUrl: './modal-link-folder.page.html',
  styleUrls: ['./modal-link-folder.page.scss'],
})
export class ModalLinkFolderPage implements OnInit {
  ...
  ngOnInit() {
    ...
    console.log('projectId: ',this.navParams.get('projectId'));
    console.log('folderId: ',this.navParams.get('folderId'));
  ...
  public async submitForm(): Promise<void> {
    ...
    if(this.navParams.get('folderId')==null ||this.navParams.get('folderId')==-1){
      this.createNewProjectLink();

    }
    else{


    this.apiService.createNewFolderLink(this.form.value).subscribe(async (result) => {
      if (result['state'] == 'CREATED') {
        
        //HACER UN CHANGE
        this.apiService.folderChanges.next({ type: 'newLink',link:result['actuProject']});
        this.isChargeLoading=false;
        this.translate.get('Link creado').subscribe((translatedText: string) => {
      ...
      else if ((result['state'] =='PROJECT_NOT_FOUND')||(result['state'] =='FOLDER_NOT_FOUND')){
        this.isChargeLoading=false;
        
        this.utilities.showToast(this.translate.instant("No se encontró la ubicación para poder crear la nueva carpeta"));
  ...
  createNewProjectLink(){
    this.apiService.createNewProjectLink(this.form.value).subscribe(async (result) => {
      if (result['state'] == 'CREATED') {
        
        //HACER UN CHANGE
        this.apiService.folderChanges.next({ type: 'newProjectLink',link:result['actuProject']});
        this.isChargeLoading=false;
        this.translate.get('Link creado').subscribe((translatedText: string) => {
          this.utilities.showToast(translatedText); 
