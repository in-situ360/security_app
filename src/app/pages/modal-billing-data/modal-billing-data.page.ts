...
@Component({
  selector: 'app-modal-billing-data',
  templateUrl: './modal-billing-data.page.html',
  styleUrls: ['./modal-billing-data.page.scss'],
})
export class ModalBillingDataPage implements OnInit {
  ...
  public resultBack:string='cantCreateBillingData'; 
    ...
    this.apiService.getEntity('UserBillingData').subscribe(
      (userBillingData: UserBillingData) => {
        ...
        if (userBillingData && userBillingData.error === 'Datos de usuario no encontrado') {
          ...
          this.resultBack='cantCreateBillingData';
        } else {
          this.resultBack='createBillingData';
