...
@Component({
  selector: 'app-interior-pnchat',
  templateUrl: './interior-pnchat.page.html',
  styleUrls: ['./interior-pnchat.page.scss'],
})
export class InteriorPnchatPage implements OnInit {
  ...
  switchLanguage(language: string) {
    ...
    this.loadingMessage = this.translate.instant('interior-pnchat.Cargando mensajes...');
    this.getMensajes();
  }
   ...
   public ngOnInit(): void {
      ...
      if(this.messagesData?.chat_id==this.idChat || mensaje.data.id==this.idChat ){
        ...
        this.ngZone.run(() => {
          ...
          setTimeout(() => {
            ...
            this.apiService.deleteEntity('mensajesPNNuevos', this.idChat).subscribe(res => {
            // console.log(res);
            }, error => {
              console.log(error);
            })
    ...
    this.notificacionesService.getObservable().subscribe((mensaje) => {
      ...
      if (this.messagesData?.chat_id == this.idChat || mensaje.data.id == this.idChat) {
          ...
          let mensajeId = Number(this.messagesData?.mensaje_id) || Number(this.messagesData?.id) || Number(mensaje.data.mensaje_id);
          ...
          // ✅ Comprobar si el mensaje ya existe en la lista
          ...
          if (mensajeExistente) {
              ...
              //this.utilities.showToast("⏩ El mensaje ya estaba en la lista, no se añadió nuevamente.");
              return;
          }
          ...
          this.ngZone.run(() => {
              ...
              console.log("MENSAJE A ENVIAR PARA EL COMPONENTE:");
              console.log(m);
              ...
              //setTimeout(() => {
                  ...
                  this.apiService.deleteEntity('mensajesPNNuevos', this.idChat).subscribe(res => {
                      console.log("🗑️ Mensajes nuevos eliminados.");
                  }, error => {
...
public getMensajes(): void {
  ...
  //this.apiService.getSubEntity('chats', this.idChat, 'mensajes').subscribe((messages: Mensaje[]) => {
    this.apiService.getPNComentarios({chatId:this.idChat,lastMessageId:this.ultimoMensajeId, language_code:this.language_code }).subscribe((/*messages: Mensaje[]*/result) => {
...
subirPdfConProgreso(file: File): Observable<any> {
 ...
 //public sendMessage(message: Mensaje): void {
  ...
  if (message.texto != "" || message.imagen || message.archive || message.audiobase64 || message.selectedFile) {
    ...
    this.apiService.addSubEntity('pnchats', this.idChat, 'mensajes', msj).subscribe((mensaje: Mensaje) => {
...
obtenerPNMensajesNuevos(){
 ...
 /* this.webInterval=setInterval(() => {
    ...
    this.apiService.obtenerPNMensajesNuevos({idChat:this.idChat}).subscribe((result) => {
          ...
          },
          {
            ...
            handler: () => {
             ...
             // this.utilities.showLoading('');
              ...
              this.apiService.deletePNChatMessage({messageId:messageId}).subscribe((result) => {
