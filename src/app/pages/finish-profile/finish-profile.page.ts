  ...
  constructor(private utilities: UtilitiesService,
    ...
    private modalController: ModalController) {
      ...
      this.form = new FormGroup({
        ...
        categoryIds:new FormControl(this.categoryIds, { validators: [this.categoryIdsValidator] }),
        ...
        validators: Validators.compose([this.surnameValidator, this.genderAndPronValidator]),
      });
    ...
    this.apiService.getEntity('user').subscribe(
      async (user: User) => {
        ...
        if(this.user.user_type_id==1){
          ...
          const toastMensaje = await this.translate.get("Nombre de Compañía").toPromise();
          this.placeholderName = toastMensaje + '*';
        }
      ...
      if(this.user.allmypronouns){
        

        this.user.allmypronouns.forEach((pronoun: any) => {
          const pron = pronoun.value;
          if (!this.prons.includes(pron)) {
            this.prons.push(pron); // Agregar pronombres predefinidos
            console.log("incluyo: ",pron);
            console.log("incluyo: ",pron);
            console.log(this.prons);
            console.log("incluyo: ");
          }
        });
        console.log(this.prons);
        this.form.get('prons')?.patchValue(this.prons); // Actualizar el formulario
      }


      if(this.user.actuallocation!=null){

        let actlo=this.user.actuallocation;
        this.originalActual = actlo.value;
        this.originalLat=actlo.lat;
        this.originalLng=actlo.lng;
        
        this.form.get('lat').patchValue(actlo.lat+'');
        this.form.get('lng').patchValue(actlo.lng+'');
        this.form.get('actual').patchValue(actlo.value); 
      ...
      async error => {
       // this.utilities.showToast('Error obteniendo el usuario');
        ...
        const toastMensaje = await this.translate.get("finish-profile.Error obteniendo el usuario").toPromise();
...
categoryIdsValidator: ValidatorFn = (_control: AbstractControl): ValidationErrors | null => {
  ...
  submitForm(){
    ...
    this.apiService.updateUser(this.form.value).subscribe(
      async (user: User) => {
        ...
        console.log(user);
        const toastMensaje = await this.translate.get("finish-profile.Usuario actualizado correctamente").toPromise();
        this.utilities.showToast(toastMensaje); 
  ...
  async createTag(){
    ...
    if (tag) {
      ...
      //this.form.get('tagInputValue')?.reset();
        ...
        const toastMensaje = await this.translate.get("finish-profile.Ya hay un Tag con ese valor").toPromise();
        this.utilities.showToast(toastMensaje); 
      }
      ...
      const toastMensaje = await this.translate.get("finish-profile.El campo está vacío, no se puede crear un Tag").toPromise();
      this.utilities.showToast(toastMensaje); 
    }
  ...
  /*validateAndSubmit(): void {
    ...
    async validateAndSubmit(): Promise<void> {
      ...
      if (this.form.invalid) {
        ...
        const toastMensaje = await this.translate.get("finish-profile.Por favor, revisa los campos con errores antes de continuar").toPromise();
        this.utilities.showToast(toastMensaje); 
  ...
  async createPron() {
    ...
    if (pron) {
      if (!this.prons.map(p => p.toLowerCase()).includes(pronLower)) {
        ...
        const toastMensaje = await this.translate.get("finish-profile.Ya hay un Pronombre con ese valor").toPromise();
        this.utilities.showToast(toastMensaje); 
      }
    } else {
      // Mostrar error de campo vacío
      
      const toastMensaje = await this.translate.get("finish-profile.El campo está vacío, no se puede crear un Pronombre").toPromise();
      this.utilities.showToast(toastMensaje); 
    }
  }
  ...
  private clearInterval() {
    ...
    if (this.interval) {
      ...
      //ENVIO TIEMPO
      this.apiService.registerTimeScreen({screenId:21,screenTime:this.countSeg}).subscribe((result) => {
  ...
  genderAndPronValidator: ValidatorFn = (form: AbstractControl): ValidationErrors | null => {
  ...
  handleGlobalClick(event: Event) {
    ...
    const clickedInsideInput = target.closest('ion-input, ion-textarea, input, textarea');
